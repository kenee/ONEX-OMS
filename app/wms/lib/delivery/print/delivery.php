...
class wms_delivery_print_delivery extends wms_delivery_print_abstract{
    ...
    public function format($print_data, $sku,&$_err,$mode='old')
    {
            ...
            foreach ($print_data['deliverys'] as $k => $dly) {
                ...
                if ($data) {
                    ...
                    if(!isset($kmtmp_shopInfo[$data['shop_id']])){
                        ...
                        $data['sender_area'] = implode('-',(array)$sender_area);
                    ...
                    foreach ($data['delivery_items'] as $k => $item)
                    {
                        ...
                        $data['delivery_items'][$k]['specifications'] = $p['specifications'];//商品规格

                        #兼容打印模板中addon商品规格字段
                        $data['delivery_items'][$k]['addon']    = $p['specifications'];//基础物料规格
            ...
            if (!empty($arrPrintProductName)) {
                ...
                foreach ($items as $k => $rows) {
                    foreach ($rows['delivery_items'] as $k2 => $v) {
                        $rows['delivery_items'][$k2]['name'] = $frontProductName[$v['bn']]['front_name'] ? $frontProductName[$v['bn']]['front_name'] : $arrPrintProductName[$rows['delivery_id']][$v['bn']]['name'];
        ...
        if($print_version=='1'){
            ...
            foreach($items as $k=>$v){
                ...
                //拣货模式合并保质期
                foreach($items[$k]['delivery_items'] as $obj_type =>$delivery_items){
                    if($obj_type == 'pick_mode_data'){
                        foreach($delivery_items as $k1=>$item){
                            if(isset($items[$k]['expire_bns'])){
                                foreach($items[$k]['expire_bns'] as $ek => $info){
                                    if($item['product_id'] == $info['bm_id']){
                                        $items[$k]['delivery_items'][$obj_type][$k1]['expire_bns'][] = $info['expire_bn'];
                                    }
                                }
                            }
                        }
                    }else{
                        //销售模式已在formatSaleStyleData方法中格式化
                    }
                }
