...
class wms_delivery_print{
    ...
    {
        $this->__printDlyObj = app::get(self::__WMS_APP)->model('delivery');
    }
    ...
    private function _checkPrintQueue() {
        ...
        if (!empty($this->__dlyIds)){
            ...
            $this->__msg['warn_msg'] = '没有发货单号被选中打印';
            return false;
        }
    ...
    private function _initData(){
        ...
        foreach($this->__deliverys['deliverys'] as $k => $dly){
            ...
            $this->__deliverys['deliverys'][$k]['print_no'] = $this->__deliverys['identInfo']['items'][$dly['delivery_id']];
        ...
        //根据发货单id取主运单号信息
        $dlyBillObj = app::get(self::__WMS_APP)->model('delivery_bill');
        ...
        //找到映射的item_id关系
        $wmsDlyItemObj = app::get(self::__WMS_APP)->model('delivery_items');
        ...
        $dlyChild_arr = kernel::database()->select('SELECT * FROM sdb_ome_delivery WHERE parent_id IN('.implode(',', $this->__outerDlyIds).')');
        ...
        //根据订单号获取订单主表信息
        $orderExtList = app::get(self::__OME_APP)->model('order_extend')->getList('order_id,receivable', [
            'order_id' => $tmp_orderIds
        ]);
        ...
        foreach($order_arr as $k =>$order){
            ...
            $order['receivable'] = $orderExtList[$order['order_id']]['receivable'];
        ...
        foreach($dlyItemsDetail_arr as $didk => $dlyItemsDetail){
            if(isset($this->__deliverys['deliverys'][$this->__outerDlyIdToWmsDlyId[$dlyItemsDetail['delivery_id']]]['delivery_items'][$dlyItemsDetail['delivery_item_id']])){
                ...
                $this->__deliverys['deliverys'][$this->__outerDlyIdToWmsDlyId[$dlyItemsDetail['delivery_id']]]['delivery_items'][$dlyItemsDetail['delivery_item_id']]['order_id'] = $dlyItemsDetail['order_id'];
