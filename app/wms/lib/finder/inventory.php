...
class wms_finder_inventory{
    ...
    function column_view($row){
        ...
        $button= <<<EOF
<a href="index.php?app=wms&ctl=admin_inventory&act=detail_inventory&p[0]=$id&view=true" class="lnk" " target="dialog::{width:700,height:600,title:'盘点查看'}">查看</a>&nbsp;
EOF;
        if ($_GET['flt'] == 'list'){

             if($row['confirm_status'] == 1){
$button.= <<<EOF
<a href="index.php?app=wms&ctl=admin_inventory&act=go_inventory&inventory_id=$id" class="lnk">加入</a>
EOF;
             }
        }else if($_GET['flt']=='confirm'){
            if($row['confirm_status'] == 1 || $row['confirm_status'] == 4){
                $button.= <<<EOF
    <a href="index.php?app=wms&ctl=admin_inventory&act=confirm_inventory&p[0]=$id&p[1]=1&find_id=$find_id" class="lnk" target="dialog::{width:700,height:600,title:'盘点'}">盘点</a>
    <a href="index.php?app=wms&ctl=admin_inventory&act=edit_inventory&inventory_id=$id&finder_id=$find_id" class="lnk" target="_blank">编辑</a>&nbsp;
EOF;
            }
            if($row['confirm_status'] == 2) {
                $button .= <<<EOF
    <a href="index.php?app=wms&ctl=admin_inventory&act=exportOne&inventory_id=$id&finder_id=$find_id" class="lnk" target="dialog::{width:400,height:200,title:'导出'}">导出</a>&nbsp;
EOF;
            }
        }
