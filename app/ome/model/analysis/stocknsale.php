...
class ome_mdl_analysis_stocknsale extends dbeav_model{
	
	var $has_export_cnf = true;
	var $export_name = '呆滞库存统计';
...
// 			$filter["{$field_name}|between"][1] =  $nsale_days_to;
    ...
    public function getExportDataByCustom($fields, $filter, $has_detail, $curr_sheet, $start, $end, $op_id){
		...
		if(!$stocknsaledata = $this->getList('*', $filter, $start, $end)){
			return false;
		}
	
		foreach($stocknsaledata as $var_data){
			// 获取科目
			$stocknsaledataRow = array(
				"material_bn" => $var_data["material_bn"],
				"material_name" => $var_data["material_name"],
				"branch_name" => $var_data["branch_name"],
			...
			foreach (explode(',', $fields) as $key => $col) {
				if(isset($stocknsaledataRow[$col])){
					$stocknsaledataRow[$col] = mb_convert_encoding($stocknsaledataRow[$col], 'GBK', 'UTF-8');
					$exptmp_data[] = $stocknsaledataRow[$col];
				}
                else
                {
                    $exptmp_data[]    = '';
