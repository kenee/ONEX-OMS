 ...
 * @Describe: 微信视频号加解密处理
 */
class ome_security_wxshipin extends ome_security_hash
{
    // 订单加密字段
    ...
    public function get_encrypt_origin($data, $type){
        // wms中，菜鸟仓不支持带*号的脱敏信息存储，传的hash_code，经yyqs测试，wms可以成功取号
        $tmpEncryptData = [];
        foreach ((array) $this->get_encrypt_fields($type) as $c => $r) {
            if ($this->get_code() == substr($data[$c], -5) && strpos($data[$c], '>>')) {
                $index = strpos($data[$c], '>>');
                $tmpEncryptData[$c] = substr($data[$c], $index+2, -5);
            }
        }
