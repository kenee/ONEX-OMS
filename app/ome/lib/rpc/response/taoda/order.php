...
class ome_rpc_response_taoda_order
{
    var $status = array(
        'TRADE_ACTIVE' => 'active',
        'TRADE_CLOSED' => 'dead',
        ...
        if($tmp = $orderObj->dump(array('order_bn'=>$order_sdf['tid']))){
            ...
            foreach($order_sdf['orders']['order'] as $v){
                ...
                foreach($v['order_items']['item'] as $items)
                {
                    ...
                    if(!isset($product_info['bm_id']) || $product_info['bm_id'] == "") trigger_error("中心不存在货品 ".$items['name']." ！",E_USER_ERROR);
                    ...
                    //更新商品的冻结库存
                    $batchList['+'][] = [
                        'bm_id' =>  $product_info['bm_id'],
                        'num'   =>  (intval($items['num'])-intval($items['sendnum'])),
                    ];
                }
                $i ++;
            }
            ...
            //如果店铺类型为淘宝、有啊、拍拍的话，查找外部会员id与当前外部id相等的
            if($shop_member_id){
                $smember = array();
    ...
    function to_delivery($result){
        ...
        foreach ($data as $item){
            ...
            foreach ($orders as $o){
                ...
                $oo['splited_num_upset_sql']   = 'IF(`splited_num` IS NULL, 1, `splited_num` + 1)';
                
                $oObj->save($oo);
                $dObj->addDelivery($order['order_id'],$dly,$ship,$order_items);
            }
            if (!$is_once){
                $dly['delivery_bn']     = $delivery_id;
                $dly['logi_no']         = $logi_no;
