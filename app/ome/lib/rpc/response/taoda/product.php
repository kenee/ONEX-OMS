...
class ome_rpc_response_taoda_product
{
    ...
    function get_pos($result){
        ...
        $sql = "SELECT p.material_bn AS bn,bp.pos_id,bp.store_position,bpp.store FROM sdb_ome_branch_pos bp 
                                JOIN sdb_ome_branch_product_pos bpp 
                                    ON bp.pos_id=bpp.pos_id 
                                LEFT JOIN sdb_material_basic_material p 
                                    ON bpp.product_id=p.bm_id 
