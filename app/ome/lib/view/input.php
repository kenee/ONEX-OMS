...
class ome_view_input{
     ...
     * @return mixed 返回值
     */
    public function input_ome_object($params)
    {

        $return_url = $params['return_url']?$params['return_url']:'index.php?app=ome&ctl=admin_crm_gift&act=object_rows';
        ...
        if(isset($params['multiple']) && $params['multiple']){
            ...
            $url = 'index.php?app=desktop&act=alertpages&goto='.urlencode('index.php?app=ome&ctl=admin_crm_gift&act=finder_common&app_id='.$app_id.'&'.$object.$getdata);
     ...
     * @return mixed 返回值
     */
    public function input_branch_object($params)
    {
        ...
        if(isset($params['multiple']) && $params['multiple']){
            ...
            $render->pagedata['_input'] = $params;
            return $render->fetch('finder/input.html','desktop');
        }else{
            if($params['value']){
                $string = $params['items'][0][$textcol];
            }else{
            ...
            $url = 'index.php?app=desktop&act=alertpages&goto='.urlencode('index.php?app=ome&ctl=admin_branch&act=finder_common&app_id='.$app_id.'&'.$object.$getdata);
            ...
            $render->pagedata['object'] = $object;
            $render->pagedata['callback'] = $callback;
            return $render->fetch('finder/input_radio.html','desktop');
        }
    }
     ...
     * @return mixed 返回值
     */
    public function input_address($params)
    {
        $regionsObj = app::get('eccommon')->model('regions');
        $regionList = $regionsObj->getlist('region_id as id,local_name as label,region_grade,case when p_region_id is null then 0 else p_region_id end as p_region_id',array('source'=>'systems'));
        $treeList = ome_func::listToTree($regionList,'id','p_region_id');
        
        $render = new base_render(app::get('desktop'));
        $render->pagedata['params'] = $params;
        $render->pagedata['treeList'] = json_encode($treeList);
        return $render->fetch('finder/input_address.html','desktop');
    }
     ...
     * @return mixed 返回值
     */
    public function input_china($params)
    {
        $regionsObj = app::get('eccommon')->model('regions');
        $regionList = $regionsObj->getlist('region_id as id,local_name as label,region_grade,case when p_region_id is null then 0 else p_region_id end as p_region_id',array('source'=>'systems'));
        $treeList = ome_func::listToTree($regionList,'id','p_region_id');
        ...
        $render = new base_render(app::get('desktop'));
        $render->pagedata['params'] = $params;
        $render->pagedata['treeList'] = json_encode(array($chinaRoot));
        return $render->fetch('finder/input_china.html','desktop');
    }
