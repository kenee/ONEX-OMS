...
class ome_autotask_timer_skufreezedecr extends ome_autotask_timer_common
{
    public function process($params, &$error_msg = '')
    {
        ...
        $now = time();
        base_kvstore::instance('ome/material/freeze/queue')->fetch('lastexectime', $lastExecTime);
        !$lastExecTime && $lastExecTime = strtotime('-5 seconds');
        if ($now - $lastExecTime < 1) {
            $error_msg = 'no info to work';
            return true; // 时间未到
        }
        base_kvstore::instance('ome/material/freeze/queue')->store('lastexectime', $now);
        ...
        // 获取未处理和失败的数据
        $selectSql = "SELECT id
            FROM sdb_ome_material_freeze_queue
        ...
        // 状态更新为……处理中
        $upStartSql = "UPDATE sdb_ome_material_freeze_queue
        ...
        // 根据pid获取数据
        $selectSql = "SELECT id, bm_id, quantity
            FROM sdb_ome_material_freeze_queue
        ...
        foreach ($groupList as $key => $item) {
            ...
            try {                
                ...
                $sql = "UPDATE `sdb_material_basic_material_stock` 
                        SET `store_freeze`=`store_freeze`+{$item['quantity']}, 
                            `last_modified`={$dateline}, 
                            `max_store_lastmodify`={$dateline}
                        WHERE `bm_id` = {$item['bm_id']} ";
                ...
                if (!$db->exec($sql) || 1 !== $db->affect_row()) {
                    ...
                    $upFailSql = "UPDATE sdb_ome_material_freeze_queue
                        SET `status`='3', `pid`='0'
                        WHERE id IN ({$ids})";
                    ...
                    // $delSql = "UPDATE sdb_ome_material_freeze_queue
                    //     SET `status`='2'
                    //     WHERE id IN ({$ids})";
                    $db->exec($delSql);
                    // $db->commit();
        ...
        // $checkSql = "SELECT * FROM sdb_ome_material_freeze_queue WHERE " . $whereAtTime . " AND status='3'";
