 ...
 */
class ome_exportextracolumn_product_storagelife_history_shipaddr extends ome_exportextracolumn_abstract implements ome_exportextracolumn_interface{
    ...
    public function associatedData($ids){
        ...
        foreach ($rs_storagelife_history as $var_rsh){
            if($var_rsh["bill_type"] == "1"){ //发货单
                ...
                $delivery = $mdl_ome_delivery->dump(array('delivery_bn'=>$var_rsh['bill_no']),'ship_addr');
                if(!empty($delivery)){
                    $tmp_array[$var_rsh[$this->__pkey]] = $delivery["consignee"]["addr"];
                }
            }elseif($var_rsh["bill_type"] == "2"){ //退换货单
                $mdl_ome_reship = app::get('ome')->model('reship');
                $rs_reship = $mdl_ome_reship->dump(array("reship_bn"=>$var_rsh['bill_no']),'ship_addr');
                if(!empty($rs_reship)){
                    $tmp_array[$var_rsh[$this->__pkey]] = $rs_reship["ship_addr"];
                }
            }
        }
        return $tmp_array;
