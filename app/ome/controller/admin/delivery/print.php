...
class ome_ctl_admin_delivery_print extends desktop_controller{
    var $name = "快递单模板";
    ...
    function index(){
        $params = array(
                    ...
                    'actions' => array(
                        array(
                            'label' => '新增模板',
                            'href' => 'index.php?app=ome&ctl=admin_delivery_print&act=addTmpl&finder_id='.$_GET['finder_id'],
                            'target' => "_blank",
                        ),
                        array(
                            'label' => '导入模板',
                            'href' => 'index.php?app=ome&ctl=admin_delivery_print&act=importTmpl',
        ...
        $this->finder('ome_mdl_print_tmpl', $params);
    }
    ...
    function addTmpl(){
        ...
        $printObj = $this->app->model('print_tmpl');
    ...
    function doUploadPkg(){
        ...
        $file = $_FILES['package'];
        $msg = kernel::single('ome_print_tmpl')->upload_tmpl($file);
    ...
    function editTmpl($tmpl_id){
        $this->begin('index.php?app=ome&ctl=admin_delivery_print');
        if ($tmpl_id == ''){
        ...
        if(!empty($data)){
            ...
            if($data['file_id']){
                $this->pagedata['tmpl_bg'] = 'index.php?app=ome&ctl=admin_delivery_print&act=showPicture&p[0]='.$data['file_id'];
            }
            //$this->pagedata['elements'] = $printObj->getElements();
    ...
    function doUploadBg(){
        ...
        //$ss->getFile("data/private/f4/a9/2b/a6c8ead623069b03458828d0ce9dbd9fb6e55483cf.php|/f4/a9/2b/a6c8ead623069b03458828d0ce9dbd9fb6e55483cf.php|filesystem","file");
  //  第一个参数是files表中的filepath,根据ID可以dump获得;
        //print_r($id);print_r($msg);
        $extname = strtolower($this->extName($_FILES['background']['name']));
        if($extname=='.jpg' || $extname=='.jpeg'){
            //$file = tempnam(ROOT_DIR.'/app/ome/upload/tmp', 'dly_');
            //unlink($file);
            //$file.='.jpg';
    ...
    function showPicture($file){
        ...
        if (!$imgContent) {
            ...
            if (!$imgContent) {
                $imgContent = file_get_contents($this->app->res_dir.'/express_bg_error.jpg');
            }
    ...
    function saveTmpl(){
        ...
        if ($re){
            ...
            if($tpl_id && $_POST['tmp_bg']){
                if(file_exists(ROOT_DIR.'/app/ome/upload/images/delivery/dly_bg_'.$tpl_id.'.jpg')){
                    unlink(ROOT_DIR.'/app/ome/upload/images/delivery/dly_bg_'.$tpl_id.'.jpg');
                }
                if(file_exists(ROOT_DIR.'/app/ome/upload/tmp/'.$_POST['tmp_bg'])){
                    rename(ROOT_DIR.'/app/ome/upload/tmp/'.$_POST['tmp_bg'],ROOT_DIR.'/app/ome/upload/images/delivery/dly_bg_'.$tpl_id.'.jpg');
                }
            }*/
            $this->end(true,'保存成功');
        }else {
    ...
    function doneUploadBg($file){
        if($file){
            $url = 'index.php?app=ome&ctl=admin_delivery_print&act=showPicture&p[0]='.$file;
            echo '<script>
