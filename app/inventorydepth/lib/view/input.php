...
class inventorydepth_view_input{

    public function __construct(&$app){
        $this->app = $app;
    }
    
    public function input_inventorydepth_object($params) 
    {
        $return_url = $params['return_url']?$params['return_url']:'index.php?app=inventorydepth&ctl=regulation_apply&act=object_rows'; 
        ...
        if(isset($params['multiple']) && $params['multiple']){
            ...
            return $render->fetch('finder/input.html','inventorydepth');
        }else{
            if($params['value']){
            ...
            $url = 'index.php?app=desktop&act=alertpages&goto='.urlencode('index.php?app=inventorydepth&ctl=regulation_apply&act=finder_common&app_id='.$app_id.'&'.$object.$getdata);
            ...
            return $render->fetch('finder/input_radio.html','inventorydepth');
        }
    }
}
