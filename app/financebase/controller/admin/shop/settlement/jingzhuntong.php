 ...
 * @version $Id: Z
 */
class financebase_ctl_admin_shop_settlement_jingzhuntong extends desktop_controller
{
    //账单类型
    ...
    public function index()
    {
        ...
        $actions[] = array(
            'label'  => '导入账单',
            'href'   => 'index.php?app=financebase&ctl=admin_shop_settlement_jingzhuntong&act=importview&type=jzt',
            'target' => "dialog::{width:500,height:200,title:'导入京准通流水'}",
        );
    ...
    public function importview()
    {
        //显示tab
        $setting = array(
            self::$_import_type => array('name'=>'支持导入.csv、.xls、.xlsx文件格式：', 'type'=>self::$_import_type),
        );
        ...
        $this->page('admin/jingzhuntong/importview.html');
    }
    ...
    public function doImport()
    {
        ...
        $url = 'index.php?app=financebase&ctl=admin_shop_settlement_jingzhuntong&act=index';
        ...
        if ($_FILES['import_file']['name'] && $_FILES['import_file']['error'] == 0) {
            ...
            if (in_array($file_type, array('csv', 'xls', 'xlsx'))) {
                ...
                if (!$move_res) {
                    ...
                    $queueData['queue_name'] = sprintf("%s_导入京准通账单文件_分派任务", $bill_date);
    ...
    public function detailed()
    {
        ...
        $actions[] = array(
            'label' => '返回列表',
            'href'  => "index.php?app=financebase&ctl=admin_shop_settlement_jingzhuntong&act=index&view={$this->pagedata['view']}&page={$this->pagedata['page']}",
        );
        
        /***
        $actions[] = array(
            'label'  => '批量确认',
            'submit' => 'index.php?app=financebase&ctl=admin_shop_settlement_jingzhuntong&act=doBatchChangeStatus&type=' . $import_res['type'] . '&finder_id=' . $_GET['finder_id'],
        );

        $actions[] = array(
            'label'  => '批量取消',
            'confirm' => '数据取消将无法恢复，需要重新导入',
            'submit' => 'index.php?app=financebase&ctl=admin_shop_settlement_jingzhuntong&act=doBatchCancel&type=' . $import_res['type'] . '&finder_id=' . $_GET['finder_id'],
        );
        ...
        ***/
        ...
        $actions[] = array(
            ...
            'target'=>'_blank',
            'class' => 'export',
            'submit' => 'index.php?app=financebase&ctl=admin_shop_settlement_jingzhuntong&act=export&type=' . $import_res['type'],
        );
    ...
    {
        $oProcess = kernel::single('financebase_data_jingzhuntong_'. $type);
        $titleList = $oProcess->getTitle();
        $titleList['title'] = implode(',', $titleList);
        $titleList['name'] = '京准通账单';
