...
class financebase_phpexcel
{
	 ...
	 * @throws Exception
	 */
	public function importExecl($file = '', $sheet = 0, $columnCnt = 0, &$options = array())
	{
	    try {
	        /* 转码 */
	 ...
	 */
	public function exportExcel($datas, $fileName = '', $options = array())
	{
	    try {
	        if (empty($datas)) {
	            return false;
     ...
     * @return mixed 返回结果
     */
    public function getTotalRow($file = '', $sheet = 0)
	{
		try {
	        /* 转码 */
	        $file = iconv("utf-8", "gb2312", $file);
	        ...
	        return $currSheet->getHighestRow();
...
//文件名称校验
        try {
            ...
            if($fileType == 'Excel2007' || $fileType == 'Xlsx') {
                ...
                header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
                header('Content-Disposition: attachment;filename="'.iconv("utf-8", "GB2312//TRANSLIT", $fileName).'.xlsx"');
                header('Cache-Control: max-age=0');
            } else { //Excel5
                $writer = IOFactory::createWriter($spreadsheet, 'Xls');
                header('Content-Type: application/vnd.ms-excel');
                header('Content-Disposition: attachment;filename="'.iconv("utf-8", "GB2312//TRANSLIT", $fileName).'.xls"');
                header('Cache-Control: max-age=0');
            }
