 ...
 * @version 0.1
 */
class financebase_autotask_task_type_cainiaoSaleImport extends financebase_autotask_task_init
{
    ...
    {
        $this->oFunc->writelog('菜鸟根据销售周期导入任务-开始', 'settlement', '任务ID:' . $task_info['queue_id']);
...
//        p($task_info);
        ...
        if ($errmsg) {
            $error_msg = $errmsg;
            $this->oFunc->writelog('菜鸟根据销售周期导入任务-部分成功', 'settlement', '任务ID:' . $task_info['queue_id']);
        } else {
            $this->oFunc->writelog('菜鸟根据销售周期导入任务-完成', 'settlement', '任务ID:' . $task_info['queue_id']);
        }
    ...
    {
        try {
            $saleMdl = app::get('financebase')->model("bill_import_sale");
            ...
            foreach ($data as $v) {
                $res = $this->saveRow($v, $importRes, $summaryMdl, $saleMdl, $type);
                if ($res['status'] == true) {
                    $not_confirm_num += 1;
     ...
     * @param mixed $saleMdl saleMdl
     * @param mixed $type type
     * @return mixed 返回操作结果
     */
    public function saveRow($data, $importRes, $summaryMdl, $saleMdl, $type)
    {
        try {
            $db = kernel::database();
            ...
            $saleRes = $this->saveSale($summaryRes, $data, $saleMdl, $importRes);
            if ($saleRes['status'] == false) {
                $db->rollback();
                return $saleRes;
            }
     ...
     * @return mixed 返回操作结果
     */
    public function saveSale($summaryRes, $data, $saleMdl, $importRes)
    {
        try {
            $saleData = array(
                'confirm_status'    => 0,
            ...
            $saleData['crc_unique'] = crc32(implode('',$saleData));

            $flag = $saleMdl->getRow('id',array('crc_unique' => $saleData['crc_unique']));
            if ($flag) {
                return array('status' => false, 'msg' => $saleData['crc_unique'] . ' 该记录已存在');
            }

            $saleData['summary_id'] =  $summaryRes['id'];
            $saleData['import_id'] =  $importRes['id'];
            $saleRes = $saleMdl->insert($saleData);
            if (!$saleRes) {
    ...
    {
        $summaryRes = $summaryMdl->getRow('*', array('type' => $type, 'import_id' => $importRes['id'], 'cost_project' => trim($data['cost_project'])));
        if (!$summaryRes) {
