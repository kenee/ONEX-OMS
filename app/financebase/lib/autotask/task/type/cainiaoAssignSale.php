 ...
 * @version 0.1
 */
class financebase_autotask_task_type_cainiaoAssignSale extends financebase_autotask_task_init
{
    ...
    public function process($task_info,&$error_msg)
    {
        ...
        $this->oFunc->writelog('菜鸟销售周期分派任务-开始','settlement','任务ID:'.$task_info['queue_id']);
...
//        $remote_url = base64_decode($task_info['queue_data']['remote_url']);
        ...
        list($status,$errmsg) = $oTask->_spliteCainiaoData($local_file,$file_type,$task_info['queue_data']['shop_id'],$task_info, $remote_url,'cainiaosaleimport');
