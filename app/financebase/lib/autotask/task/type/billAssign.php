 ...
 * @version 0.1
 */
class financebase_autotask_task_type_billAssign extends financebase_autotask_task_init
{
    ...
    public function process($task_info,&$error_msg)
    {
        ...
        $this->oFunc->writelog('对账单分派任务-开始','settlement','任务ID:'.$task_info['queue_id']);
        ...
        $task_name = sprintf("%s_%s",$task_info['queue_data']['shop_name'],$task_info['queue_data']['bill_date']);
        
        list($status,$errmsg) = $oTask->_spliteData($local_file,$file_type,$task_info['queue_data']['shop_id'],$task_name,$task_info);

        unlink($local_file); $storageLib->delete($remote_url);

        if($status){
            $this->oFunc->writelog('对账单分派任务-完成','settlement','任务ID:'.$task_info['queue_id']);
        }else{
            $error_msg[] = $errmsg; 
            $this->oFunc->writelog('对账单分派任务-失败','settlement','任务ID:'.$task_info['queue_id']);
            return false;
        }
