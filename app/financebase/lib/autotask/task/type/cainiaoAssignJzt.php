 ...
 * @version $Id: Z
 */
class financebase_autotask_task_type_cainiaoAssignJzt extends financebase_autotask_task_init
{
    /**
     * 处理
    ...
    public function process($task_info, &$error_msg=null)
    {
        ...
        //切片任务
        list($status,$errmsg) = $oTask->_spliteCainiaoData($local_file,$file_type,$task_info['queue_data']['shop_id'],$task_info, $remote_url,'cainiaojztimport');
        ...
        //return
        if($status){
            $this->oFunc->writelog('京准通分派任务-完成','settlement','任务ID:'.$task_info['queue_id']);
        }else{
            $error_msg[] = $errmsg; 
            $this->oFunc->writelog('京准通分派任务-失败','settlement','任务ID:'.$task_info['queue_id']);
            return false;
        }
