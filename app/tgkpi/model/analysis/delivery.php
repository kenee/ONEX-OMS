...
class tgkpi_mdl_analysis_delivery extends dbeav_model{
    ...
    public function getlist($cols='*', $filter=array(), $offset=0, $limit=-1, $orderType=null){
        ...
        $sql = "SELECT pick_owner as picker, op_name, COUNT(DISTINCT delivery_id) as deliveryTotal , SUM(pick_num) as pickTotal , pick_error_num from ".$this->realTableName." where ";
        $sql .= $this->pickFilter($filter);
        $sql .= ' GROUP BY pick_owner';
        if($orderType){
        ...
        foreach ($all as $key=>$value) {
            ...
            $data[$key]['op_name']                         = $value['op_name'];
    ...
    public function fetch_graph_data($params){
        ...
        $time_range = [];
        if($params['target'] == 2):
            $sql = "SELECT DATE_FORMAT(FROM_UNIXTIME(pick_start_time),'$time_format') as date,op_name,sum(pick_num) as pick_nums
            FROM sdb_tgkpi_pick
            WHERE pick_status='deliveryed' AND pick_start_time between ".strtotime($params['time_from'])." AND ".strtotime($params['time_to'])."
            GROUP BY DATE_FORMAT(FROM_UNIXTIME(pick_start_time),'$time_format'),op_name
            ";
            $rs = $this->db->selectLimit($sql,100,0);
            foreach((array)$rs as $v) {
                if(!in_array($v['date'],$time_range)) $time_range[] = $v['date'];
                $data[$v['op_name']][$v['date']] = $v['pick_nums'];
            }
        else:
            $sql = "SELECT DATE_FORMAT(FROM_UNIXTIME(pick_start_time),'$time_format') as date,op_name,count(distinct delivery_id) as deliverys
            FROM sdb_tgkpi_pick
            WHERE pick_status='deliveryed' AND pick_start_time between ".strtotime($params['time_from'])." AND ".strtotime($params['time_to'])."
            GROUP BY DATE_FORMAT(FROM_UNIXTIME(pick_start_time),'$time_format'),op_name
            ";
            ...
            foreach((array)$rs as $v) {
                if(!in_array($v['date'],$time_range)) $time_range[] = $v['date'];
                $data[$v['op_name']][$v['date']] = $v['deliverys'];
            }
        endif;
