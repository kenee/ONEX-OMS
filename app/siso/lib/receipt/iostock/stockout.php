...
class siso_receipt_iostock_stockout extends siso_receipt_iostock_abstract implements siso_receipt_iostock_interface{
    /**
     * 
     * 出入库类型id
     * @var int
    ...
    function get_io_data($data){
        ...
        if ($iso_items_detail){
            foreach ($iso_items_detail as $k=>$v){
                ...
                $iostock_data[] = $tmp;
                if(!in_array($this->_typeId,array('10'))){#采购退货出库
                    $tmp['original_id'] = $iso_id;
                    $tmp['original_bn'] = $iso_detail['iso_bn'];
                }
                $batch_data[] = $tmp;
            }
            $this->dealBatch($batch_data, ['returned_purchase','iso'], '-');
        }
    ...
    public function create($params, &$data, &$msg=null){
        $result = parent::create($params, $data, $msg);
         if(in_array($this->_typeId,array('10','7','700'))){
            $io = '0';
