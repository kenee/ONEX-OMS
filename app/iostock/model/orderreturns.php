...
class iostock_mdl_orderreturns extends iostock_mdl_iostock{
...
//判断是否为本操作所需的csv文件
    ...
    function prepared_import_csv_row($row,$title,&$tmpl,&$mark,&$newObjFlag,&$msg){
        ...
        if( substr($row[0],0,1) == '*' ){
            ...
            if( $row[0]){
               ...
               if(array_key_exists('*:销售出库单号',$title)){ //主表记录
                    ...
                    isset($this->delivery) ? '' : $this->delivery = $row[1];  //发货单号
                    if($this->mainItem){
                   ...
                   $this->io_title('main');
                    ...
                    if(isset($this->detail_Items)){
                        ...
                        if($this->detail_Items > 100){
                            ...
                            $msg['error'] = "一次退货过多，请减少到100条明细情况以下！";
                            return false;
                        }
                    } else {
                        $this->detail_Items = 1;
                    ...
                    //校验明细项目数据是否存在
                    $bn = app::get('material')->model('basic_material')->dump(array('material_bn'=>$row[0]));
                    ...
                    if(empty($pro)){ //检验退货商品是否存在于销售出库单中
                    ...
                    if(!is_numeric($row[2])){
                        ...
                        return false;
                    }elseif(empty($nums) or ($nums['nums'] < $row[2])){ //检验退货数量是否大于原有数量
     ...
     function finish_import_csv(){
        ...
        $type = ome_iostock::RETURN_STORAGE;
