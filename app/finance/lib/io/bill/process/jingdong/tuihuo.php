...
class finance_io_bill_process_jingdong_tuihuo{
        ...
        if( $row == null ){
            ...
            if( $row['group'] == 1 ){
                if($row['data']['order_bn'] != '合计'){
                    ...
                    //     $sdf5['fee_item'] = '手续费';
                    //     $sdf5['unique_id'] = finance_func::unique_id(array(
                    //         $row['data']['date'],
                    //         $row['data']['order_bn'],
                    //         $row['data']['goods_bn'],
                    //         $sdf5['fee_item']
    ...
    private function field_verify($row,$mdl){
        ...
        #日期验证
        if($data['date']!=''){
            $rs = finance_io_bill_verify::isDate($data['date'],'退货时间格式错误');
            if($rs['status'] == 'fail'){
                $result['msg'][ $inline ] .= $rs['msg'].'|';
                $result['status'] = 'fail';
            }
