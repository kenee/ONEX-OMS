...
class erpapi_shop_matrix_tmall_request_delivery extends erpapi_shop_request_delivery
{
    protected function get_delivery_apiname($sdf)
    {
        ...
        //喵住发货
        if (isset($sdf['orderinfo']['order_source']) && $sdf['orderinfo']['order_source'] == 'miaozhu') {
            $api_name = SHOP_FX_JZ_LOGISTICS_OFFLINE_SEND;
        }
    ...
    protected function get_confirm_params($sdf)
    {
        ...
        if($sdf['delivery_package']){
            $packageInfo = $this->_getDeliveryPackages($sdf, 'tmall');
        ...
        if(($sdf['is_split'] == 1 || $is_packages) && $sdf['orderinfo']['is_modify'] != 'true') {
            ...
            if($packageInfo){
                ...
                //按发货子订单明细
                $deliveryItems = $this->_getDeliveryItems($sdf, 'tmall');
                if($deliveryItems){
