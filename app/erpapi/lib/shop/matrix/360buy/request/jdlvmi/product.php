 ...
 * @describe 处理店铺商品相关类
 */
class erpapi_shop_matrix_360buy_request_jdlvmi_product extends erpapi_shop_request_product {
    ...
    public function itemsAllGet($filter,$offset=0,$limit=100)
    {
        ...
        $result = $this->__caller->call(SHOP_JDLVMI_GET_ITEMS_ALL_RPC,$param,array(),$title,$timeout,$shop_bn);
        ...
        if($result['data']) {
            ...
            if($jos_result_dto['count']) $totalResults = $jos_result_dto['count'];

            $jos_data = array('totalResults'=>$totalResults);
            $item = array();
            if($jos_result_dto){
