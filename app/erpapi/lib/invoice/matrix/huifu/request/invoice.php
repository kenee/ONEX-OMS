 ...
 * 汇付电子发票请求业务
 */
class erpapi_invoice_matrix_huifu_request_invoice extends erpapi_invoice_request_invoice
{
    /**
     * 开票推送是否异步请求
     * @var bool
    ...
    public function create($sdf)
    {
        ...
        $title = "汇付电子发票开蓝票-" . $order['invoice_apply_bn'];
    ...
    public function create_result($sdf)
    {
        ...
        $result = $this->__caller->call($apiname, $params, null, '汇付电子发票获取发票开票结果', 20, $sdf["invoice_apply_bn"]);
    ...
    public function cancel($sdf)
    {
        ...
        $params['data']  = json_encode($requestParams);
        $params['taxNo'] = $order['tax_no'];
        $title           = "汇付电子发票冲红-" . $order['invoice_apply_bn'];
    ...
    #版式文件生成（汇付生成板式文件的时候，已立即返回了下载地址，所以直接用生成接口查下载地址，代替板式文件查询接口）
    ...
    public function createPdfFile($sdf)
    {
        ...
        $params['data']  = json_encode($requestParams);
        $params['taxNo'] = $order['tax_no'];
        $title           = "汇付电子发票红票生成版式文件-" . $order['invoice_apply_bn'];
