 ...
 * [汇付]电子发票渠道
 */
class invoice_event_trigger_data_huifu extends invoice_event_trigger_data_common
{
    /**
     * 发票材质映射
     * @var string[]
    ...
    public function getEinvoiceRequestParams($orderInfo, $einvoice_type = 'blue', $source = 'b2c')
    {
        ...
        //绑定关系
        $result = kernel::single('erpapi_router_request')->set('bind', 'huifu')->bind_bind($channelExtendData);
        if (!$result) {
            $error_msg        = '绑定关系失败!';
        ...
        $params = [
            ...
            'remarks'            => $orderInfo["remarks"], // 备注:长度为30个字符
...
//            'invoiceNo'        => $orderInfo['ship_bank'], // 发票号码
    ...
    {
        $sdf = parent::getUploadParams($invoice, $electronic);
        $sdf['invoice']['channel_type'] = 'huifu';
        return $sdf;
    }
