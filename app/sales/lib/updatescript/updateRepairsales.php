 ...
 * @author
 **/
class sales_updatescript_updateRepairsales
{
...
*/
    ...
    function getsale_data($Oiostock,&$data,$offset,$pre_time,$last_time){
        ...
        if($pre_time!=''&&$last_time!=''){
           $where = ' o.create_time >='.$pre_time.' and o.create_time<='.$last_time.' and o.type_id = 3';
        }else{
           $where = ' o.create_time<='.$last_time;
        }


        @ini_set('memory_limit','1024M');
        @set_time_limit(0);
        $sql = 'select o.iostock_bn from sdb_ome_iostock as o LEFT JOIN sdb_ome_sales as s on o.iostock_bn=s.iostock_bn where '.$where.' AND s.sale_id IS null limit '.$offset*$limit.','.$limit;
