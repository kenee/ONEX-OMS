...
class sales_exportextracolumn_sales_shipaddr extends sales_exportextracolumn_abstract implements sales_exportextracolumn_interface{
    ...
    public function associatedData($ids){
        ...
        unset($shipaddr_lists);
        $archive_ordObj = kernel::single('archive_interface_orders');
        $archiveshipaddr_lists = $archive_ordObj->getOrder_list(array('order_id'=>$ids),'order_id,ship_addr');
        foreach($archiveshipaddr_lists as $k=>$row){
            $tmp_array[$row[$this->__pkey]] = $row['ship_addr'];
        }
        unset($archiveshipaddr_lists);
        return $tmp_array;
    }
