...
class vop_aftersales
{
    ...
    {
        $aftersalesMdl = app::get('vop')->model('aftersales');
        ...
        $items = app::get('vop')->model('aftersales_items')->getList('*', ['aftersale_id'  => $aftersale_id]);
        
        $sdf                    = [];
        $sdf['trade_time']      = $aftersale['aftersale_time']; //售后时间
        $sdf['delivery_time']   = strtotime($aftersale['at_time']); //收货时间
        $sdf['type']            = kernel::single('finance_iostocksales')->get_sales_type('JIT_STOCKIN'); //业务类型 todo
        $sdf['order_bn']        = $aftersale['po_bn']; //业务单据bn
