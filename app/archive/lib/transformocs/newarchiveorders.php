...
class archive_transformocs_newarchiveorders  extends archive_transformocs_abstract
{ 
        ...
        for($i=1;$i<=$page;$i++){
            ...
            foreach ( $ordersdfList as $ordersdf ) {
                ...
                if ($order_result) {
                    //生成发货单
                    $order_id = $ordersdf['order_id'];
                    $delivery_list = $this->_tranDeliverys($order_id);
                    if ($delivery_list) {

                        foreach ($delivery_list as $delivery ) {
                            $delivery_result = $this->addDelivery($order_id,$delivery);
                            if (!$delivery_result) {
                                $fail++;
                                $this->db->rollBack();
                            }
                    ...
                    $this->db->commit();
                    //成功订单
                    error_log($order_bn.",",3,DATA_DIR.$_SERVER['SERVER_NAME'].'.succ.log');
                    $succ++;
                }else{
                    //失败订单
                    $fail++;
                    $this->db->rollBack();
                    error_log($order_bn.",",3,DATA_DIR.$_SERVER['SERVER_NAME'].'.fail.log');
                }
            }
    ...
    public function _getorderObject($order_ids)
    {
        ...
        $item_keys = 'order_id,obj_id,item_id,shop_goods_id,product_id,shop_product_id,bn,`name`,cost,price,amount,pmt_price,sale_price,nums,sendnum,item_type,`delete`';
     ...
     * @author sunjing@shopex.cn
     */
    function _tranDeliverys($order_id)
    {
        
        $delivery_list = array();
        $deliveryquery = mysql_query("SELECT d.* FROM sdb_ome_delivery_order  as od LEFT JOIN sdb_ome_delivery as d on od.delivery_id=d.delivery_id  WHERE od.order_id='".$order_id."' group by d.delivery_id",$this->conn);
        ...
        while($delivery_detail = mysql_fetch_assoc($deliveryquery) ){
            ...
            if ($delivery_detail) {
                ...
                $itemsquery = mysql_query("SELECT item_id,delivery_id,product_id,shop_product_id,bn,product_name,number,verify,verify_num FROM sdb_ome_delivery_items WHERE delivery_id=".$delivery_id,$this->conn);
                $items = array();
                ...
                while ($items_detail = mysql_fetch_assoc($detail_query) ) {
                    $products  = $this->getProduct($item_list['bn']);
                    $items_detail['product_id'] = $products['product_id'];
