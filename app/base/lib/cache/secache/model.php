...
if(!defined('SECACHE_SIZE')){
    define('SECACHE_SIZE','100M');
}

class base_cache_secache_model extends base_cache_abstract
{
    ...
    function workat($file){
        ...
        if(!file_exists($this->_file)){
            ...
            if (version_compare(phpversion(), '5.5', '>=')){
                $unpack_key = 'V1max_size/Z*ver';
            }else{
                $unpack_key = 'V1max_size/a*ver';
            }
            $info = unpack($unpack_key,fread($this->_rs,$this->info_size));
    ...
    function _format($truncate=false){
        if($this->lock(true,true)){
            ...
            $this->_puts($this->header_padding,pack($pack_key,$this->max_size,$this->ver));
