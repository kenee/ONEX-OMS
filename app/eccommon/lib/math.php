...
class eccommon_math{
    ...
    public function __construct(){
        $this->operationDecimals = app::get('eccommon')->getConf('site.decimal_digit.count');      		//运算精度
        $this->operationCarryset = app::get('eccommon')->getConf('site.decimal_type.count');      		//运算进位方式
        $this->goodsShowDecimals = app::get('eccommon')->getConf('site.decimal_digit.display'); 			//运算显示精度
		$this->goodsShowCarryset = app::get('eccommon')->getConf('site.decimal_type.display'); 			//运算显示进位方式
     ...
     * @return float 进位后 运算 价格
     */
    public function getOperationNumber( $number = 0, $is_count=true ){
		...
		if ($is_count)
		{
			$result = call_user_func_array( $this->operationFunc , array(floatval($number) * pow( 10 , $this->operationDecimals)) );
			if (function_exists("bcdiv"))
     ...
     * @param mixed $display_decimals display_decimals
     * @param mixed $operation_carryset operation_carryset
     * @return mixed 返回值
     */
