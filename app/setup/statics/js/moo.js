...
var Event = new Native({
	...
	initialize: function(event, win){
		...
		this.$extended = true;
		var type = event.type;
		var target = document.id(event.target || event.srcElement);
...
var clean = function(item, retain){
	...
	if (!uid) return;
    /*redo*/
    for(item in storage[uid]){
       if($type(storage[uid][item])=='element'){
           storage[uid][item].remove();
       }
    }
...
Element.implement({
    ...
    toQueryString: function(filterEl){
		var queryString = [];
   
		this.getElements('input, select, textarea', true).each(function(el){
			if (!el.name || el.disabled || el.type == 'submit' || el.type == 'reset' || (el.type == 'file' && el.value.trim()=='')) return;
            if(filterEl){
