...
require_once(dirname(__FILE__) . '../../../base/lib/db/compat.php');
class setup_serverinfo 
{
        ...
        foreach(get_class_methods($this) as $func){
            if(substr($func,0,5)==$func_prefix){
                ...
                if($result['items']){
                    ...
                    $group[$result['group']]['items'] = array_merge(is_array($result['group'])&&$group[$result['group']['items']]?$group[$result['group']['items']]:array(),$result['items']);
    ...
    function ecae_server_req(&$socre) 
    {
        ...
        $rst = defined('ECAE_MONGO_HOST');
        $items['Mongo'] = array(
            'value' => $rst ? '已经激活 Mongo' : '请激活 Mongo',
            'result'=> $rst,
        );
        if(!$rst)   $allow_install = false;

        $res = ecae_api_call('storage_list_group');

        $rst = (is_array($res) && in_array('private:private', $res)) ? true : false;
        ...
        $rst = (is_array($res) && in_array('public:public', $res)) ? true : false;
        ...
        $rst = (is_array($res) && in_array('images:public', $res)) ? true : false;
    ...
    function test_server_req(&$score){
        ...
        $rst = version_compare(PHP_VERSION,'7.4','>=');
        $items['PHP5以上'] = array(
        ...
        if(!$rst){
            ...
            $rst = false;
            // Prefer installer input, then environment overrides, then existing constants
            $envDbHost = getenv('ONEX_OMS_DB_HOST');
            $envDbUser = getenv('ONEX_OMS_DB_USER');
            $envDbPassword = getenv('ONEX_OMS_DB_PASSWORD');
            if ($envDbUser && !defined('DB_USER')) {
                define('DB_USER', $envDbUser);
            }
            if ($envDbPassword && !defined('DB_PASSWORD')) {
                define('DB_PASSWORD', $envDbPassword);
            }
            if ($envDbHost && !defined('DB_HOST')) {
            ...
            // elseif($db_ver = @mysql_get_server_info($rs)){
            //     define('DB_HOST','');
            // }
            else{
        ...
        */
        ...
        $rst = extension_loaded('xlswriter');
        $items['是否正常安装xlswriter扩展'] = array(
            'value'=>$rst?'成功':'失败 (将影响程序解码及TASK任务功能)',
            'result'=>$rst,
        );

        $rst = extension_loaded('swoole');
        $items['是否正常安装swoole扩展'] = array(
            'value'=>$rst?'成功':'失败 (将影响导入导出功能)',
            'result'=>$rst,
        );
