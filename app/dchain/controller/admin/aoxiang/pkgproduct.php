 ...
 * @version 2023.03.08
 */
class dchain_ctl_admin_aoxiang_pkgproduct extends desktop_controller
{
    var $title = '组合商品列表';
    ...
    {
        $actions = array();
        $base_filter = array('product_type'=>'combine');
    ...
    public function single_sync()
    {
        ...
        //row
        $rowInfo = $this->_mdl->dump(array('pid'=>$id, 'product_type'=>'combine'), '*');
        if(empty($rowInfo)){
            $this->splash('error', $url, '没有捆绑商品关联信息');
        }
        
        if(!in_array($rowInfo['sync_status'], array('none', 'fail', 'running'))){
            $this->splash('error', $url, '捆绑商品已经同步成功,不能重复同步');
        }
        
        if(empty($rowInfo['shop_iid'])){
            $this->splash('error', $url, '捆绑商品没有关联平台店铺商品ID,不能同步');
        }
        ...
        $result = $this->_axProductLib->syncCombineProduct($dataList, $operation);
        if($result['rsp'] != 'succ'){
    ...
    public function ajaxSync()
    {
        ...
        foreach ($dataList as $key => $row)
        {
            ...
            if(empty($row['shop_iid'])){
                //error
                $retArr['ifail'] += 1;
                $retArr['err_msg'][] = '单据：'. $row['product_bn'] .'捆绑商品没有关联平台的店铺商品ID,不能同步';
    ...
    public function single_mapping_sync()
    {
        ...
        if(!in_array($rowInfo['sync_status'], array('succ')) || in_array($rowInfo['mapping_status'], array('succ'))){
            $this->splash('error', $url, '捆绑商品同步状态,不允许同步商品关系,请检查!');
        }
        ...
        if(empty($rowInfo['shop_iid']) || $rowInfo['sync_status']=='lack'){
            ...
            $this->splash('error', $url, '捆绑商品没有关联平台店铺的商品ID,不能同步!');
        }
    ...
    public function ajaxMappingSync()
    {
        ...
        foreach ($dataList as $key => $row)
        {
            ...
            if(empty($row['shop_iid']) || $row['sync_status']=='lack'){
                //error
                $retArr['ifail'] += 1;
                $retArr['err_msg'][] = '单据：'. $row['product_bn'] .'捆绑商品没有关联平台店铺的商品ID,不能同步';
    ...
    public function batchApportion($shop_id)
    {
        ...
        $this->display('aoxiang/batch_apportion_combine_product.html');
    }
    ...
    public function plusSync($shop_id)
    {
        ...
        $this->display('aoxiang/batch_plus_sync_combine_product.html');
    }
    ...
    public function plusSyncProduct($params, $page)
    {
        ...
        //filter
        $filter = array('shop_id'=>$shop_id, 'sync_status'=>$sync_status, 'product_type'=>'combine');
        ...
        $queueData = array(
            ...
            'worker'=> 'dchain_product.assignCombineProduct',
        );
    ...
    public function plusMapping($shop_id)
    {
        ...
        $this->display('aoxiang/batch_plus_mapping_combine_product.html');
    }
