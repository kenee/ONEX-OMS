...
var editor_style_1 =new Abstract({
	style_init:function(){
        ...
        if(this.inc.doc.body.innerHTML.substr(0,6)=='&nbsp;'){
		   ...
		   var obj=this;
		   //this.mce_handle.setOpacity(1);
		   $ES('img',this.mce_handle).addEvent('click',function(){this.inc.win.focus();}.bind(this));
		   ...
		   $E('.ft_select',this.mce_handle).addEvent('change',function(e){
		         var v=this.getValue();
		         if(!v)return;
                 if(window.ie&&obj.range)obj.range.select();
				 obj.set('fontName',v);
		   });
		   ...
		   $$($E('.fontColorPicker',this.el),
		   $E('.fontBGColorPicker',this.el),
		   $E('.ft_select',this.mce_handle),
		   $E('.fs_select',this.mce_handle)
		   ).addEvent('click',function(){
		           this.range=null;
				    if(window.ie&&this.getSelection().type.toLowerCase()!='none'){
					    this.range = this.getRange()
					}
					if(!window.ie){this.range = this.getRange();}
		   }.bind(this));

		   Ex_Loader("picker",function(){
			   new GoogColorPicker($E('.fontColorPicker',this.el),{
					 onSelect:function(hex,rgb,e){
						  if(window.ie&&this.range)
						   this.range.select();
	...
	set:function(cmd,arg){
	    ...
	    if(window.ie){this.inc.win.focus();}
		this.exec(cmd,arg);
		try{
