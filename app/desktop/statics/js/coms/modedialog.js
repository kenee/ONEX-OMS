        ...
        initialize:function(url,options){
            ...
            if(window.location.hash) {
                ...
                for (var i = 0; i < arr.length; i++) {
                    ...
                    if(decodeURIComponent(tmp_arr[0]) == '_finder[finder_id]') {
                        url += '&finder_id='+tmp_arr[1];
                        break;
                    }
                    if(decodeURIComponent(tmp_arr[0]) == 'finder_id') {
                        url += '&finder_id='+tmp_arr[1];
                        break;
                    }
                }
            ...
            var config="modal=yes;dialogWidth={width}px;dialogHeight={height}px;resizable={resizable};status=no;";
            config=config.substitute(options);
            this.fireEvent('init');
            window.modedialogInstance = this;
            this.openwin = window.open(url,options.title,config);
