...
initialize:function(seri,options){
    ...
    var state=0;
    var frm=this.frm=new Element('iframe',{'src':DESKTOPRESFULLURL+'/about.html','id':name+"_frm",'name':name+"_frm",'frameborder':0,'border':0}).addEvent('load',function(){
             if(state)return;
             state=1;
             var editor=that;   
	         ...
	         var documentTemplate =   ''
									...
									+'</head><body spellcheck="false" id="'+seri+'" style="break-word:break-all;word-wrap:break-word;">'
   ...
   if (!this.busy) {
	...
	switch(cmd){
		...
		case "insertHTML":
                ...
                if(window.ie){
                         this.inc.win.focus();
						 if(!this.getRange().pasteHTML)return;
...
dialog:function(action,options){
	...
	var _this=this;
	if ('image'==action)
	 return Ex_Loader('modedialog',function(){
			new imgDialog(url,{onCallback:function(image_id,image_src){						
...
insertImg:function(image_id,image_src,center){
	...
	var img=new Element('img',{src:image_src}),imgID='img'+$uid(img);
    ...
    win_img.removeAttribute('turl','id');	
},
