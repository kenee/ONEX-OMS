...
class material_mdl_basic_material_cat extends dbeav_model{
    
    var $table_name = 'sdb_material_basic_material_cat';
   ...
   /**
    ...
    public function save(&$aData,$mustUpdate = null,$mustInsert = false)
    {
        ...
        if($parent_changed == true){
            $rs = $this->getList('cat_id,cat_path,parent_id',array('cat_path|has'=>','.$aData['cat_id'].','));
            foreach($rs as $v){
                $save_data = array(
                    'cat_id' => $v['cat_id'],
                    'cat_path' => $this->getCatPath($v['parent_id']),
                );
    ...
    public function getTree(){
        return $this->db->select('SELECT o.cat_name AS text,o.cat_code AS code,o.cat_id AS id,o.parent_id AS pid,
                    o.p_order,o.cat_path,is_leaf,o.type_id as type,
                    o.child_count,o.tb_cat_name,o.tb_cat_path,o.tb_cid 
        ...
        if($data){
            ...
            foreach($data as $i=>$value){
                ...
                $tmpCat[$value['pid']][] =array(
                    ...
                    'tb_cat_name'=>$data[$i]['tb_cat_name'],
                    'tb_cat_path'=>$data[$i]['tb_cat_path'],
                    'tb_cid'=>$data[$i]['tb_cid'],
                    'cls'=>$cls,
                    'url'=>$link
                );
            }
    ...
    function cat2json($return=false)
    {
        ...
        base_kvstore::instance('material')->store('basic_material_cat.data',$contents);
        app::get('material')->setConf('kvstore_goods_cat_expires',time());        
        if($return){
            return $contents;
        }else{
            return true;
    ...
    {
        if( base_kvstore::instance('material')->fetch('basic_material_cat.data', $contents) !== false ){
    ...
    {
        $catid = intval($catid);
		$aCats = $this->getList('*', array('parent_id'=>$catid));
        if(count($aCats) > 0){
		...
		$basic_material_mdl = $this->app->model('basic_material');
		$material_count = $basic_material_mdl->count(array('cat_id'=>$catid,'disabled'=>'false'));
        if($material_count > 0){
            //trigger_error(app::get('b2c')->_('删除失败：本分类下面还有商品'), E_USER_ERROR);
            $msg = '删除失败：分类下存在基础物料，不能删除';
            return false;
        }
        
        //$row = $this->db->selectrow('SELECT parent_id FROM sdb_b2c_goods_cat WHERE cat_id='.intval($catid));
		$row = $this->getList('parent_id', array('cat_id'=>$catid));
        $parent_id = $row[0]['parent_id'];

        $this->delete(array('cat_id'=>$catid));
        $sql = 'UPDATE '.$this->table_name.' SET child_count = child_count-1 WHERE cat_id='.intval($parent_id);
        $this->db->exec($sql);
        $this->cat2json();
        base_kvstore::instance('material')->store('basic_material_cat.num', 0);
        return true;
    }

    function get_new_cat($limit)
    {
        $cat_id = $this->db->select('SELECT cat_id, MIN(bm_id) AS bm_id FROM `sdb_material_basic_material` where cat_id <> \'0\' group by cat_id order by bm_id desc limit 0,'.$limit);
        if(is_array($cat_id)){
    ...
    {
		$list = $this->getRow("cat_name",array('cat_id'=>$cat_id));
    ...
    function init_data()
    {
        base_kvstore::instance('material')->fetch('basic_material_cat.num', $num);
        if(! $num ){
            $num = $this->count();
            if( $num == 0){
                $num = 1;
            ...
            base_kvstore::instance('material')->store('basic_material_cat.num', $num);
        }
    }
