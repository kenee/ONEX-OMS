...
class material_view_input{
    ...
    /**
     * input_material_object
     * @param mixed $params 参数
     * @return mixed 返回值
     */
    public function input_material_object($params) 
    {
        ...
        $app = app::get($app_id);
        $o = $app->model($object);
        $app_desktop = app::get('desktop');
        $render = new base_render($app_desktop);
        ...
        if(isset($params['multiple']) && $params['multiple']){
            ...
            return $render->fetch('finder/input.html','material');
        }else{
            if($params['value']){
            ...
            $url = 'index.php?app=desktop&act=alertpages&goto='.urlencode('index.php?app=material&ctl=admin_material_sales&act=finder_common&app_id='.$app_id.'&'.$object.$getdata);
            ...
            return $render->fetch('finder/input_radio.html','material');
        }
    }
    ...
    function input_category($params)
    {
        ...
        return $render->fetch('admin/material/category/input_category.html');
    }
}
