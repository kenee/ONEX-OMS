 ...
 */
/**
 * @description 门店配送短信触发事件
 * @access public
 * @param void
 * @return void
 */
class  taoexlib_sms_event_o2oship extends taoexlib_sms_event_abstract implements taoexlib_sms_event_interface
{
     ...
     * @var string
     */
    protected $_tmplContent = '尊敬的用户{收货人}，您的包裹已到达{门店名称}，门店联系电话{门店联系电话}，请耐心等待，我们会尽快为您进行配送，请凭借提货单{提货单}和校验码{校验码}进行签收, 谢谢！{短信签名}';
    ...
    protected function getTmplVariables(){
        return array(
            ...
            array('id' => 'mendianlianxidianhua', 'name' => '门店联系电话', 'value' => '{门店联系电话}','br'=>''),
            array('id'=>'msgsign','name'=>'短信签名','value'=>'{短信签名}','br'=>'<br>'),
    ...
    public function checkParams(&$params, &$error_msg){
        ...
        if(!$params["ship_mobile"] || !$params["ship_name"] || !$params["store_name"]  || !$params["store_contact_tel"] || !$params["pickup_bn"]){
            $error_msg = '收货人/联系电话/门店名称/门店电话/提货单不能为空';
            return false;
        }
    ...
    public function formatContent($params, &$sendParams, &$error_msg){
        ...
        if(!$content){
            $contentinfo = $rule_sample_mdl->getOtherSmsContent('o2oship');
        ...
        $find = array('{收货人}','{门店名称}','{短信签名}','{门店联系电话}','{提货单}','{校验码}');
        $replace = array($helper->str_mosaic($ship_name,'*',true,0,1), $store_name, $sendParams['replace']['msgsign'], $store_contact_tel,'******',$mobile_pkcode_display);
