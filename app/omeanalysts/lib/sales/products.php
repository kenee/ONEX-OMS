 ...
 */
/**
 * 基础物料按照(店铺 + 销售物料 + 基础物料)的纬度统计数据
 * @todo：按照(店铺 + 销售物料 + 基础物料)的纬度统计数据;
 *
 * @author wangbiao<wangbiao@shopex.cn>
 * @version 2022.06.02
 */
class omeanalysts_sales_products extends eccommon_analysis_abstract implements eccommon_analysis_interface
{
    ...
    public function finder()
    {
        ...
        $params =  array(
            'model' => 'omeanalysts_mdl_sales_products',
            'params' => array(
                'actions'=>array(
                    array(
                        ...
                        'class'=>'export',
                        'icon'=>'add.gif',
                        'href'=>'index.php?app=omeanalysts&ctl=ome_analysis&act=products&action=export&material_type=sales_and_basic_material',
                        'target'=>'{width:600,height:300,title:\'生成报表\'}'),
                ),
                'title'=>app::get('omeanalysts')->_('销售明细统计<script>if($$(".finder-list").getElement("tbody").get("html") == "\n" || $$(".finder-list").getElement("tbody").get("html") == "" ){$$(".export").set("href","javascript:;").set("onclick", "alert(\"没有可以生成的数据\")");}else{$$(".export").set("href",\'index.php?app=omeanalysts&ctl=ome_analysis&act=products&action=export&material_type=sales_and_basic_material\');}</script>'),
