...
class omeanalysts_crontab_script_analysisProductNsale{

    /**
     * analysisProductNsale
     * @return mixed 返回值
     */
    public function analysisProductNsale(){
        @set_time_limit(0);
        $db = kernel::database();
        
        //clear table
        $clear_sql = "truncate table ".kernel::database()->prefix."ome_analysis_productnsale";
        $db->exec($clear_sql);
        
        $today_time = strtotime(date('Y-m-d',time())); //如1点调用 今天0点的时间戳
        $filter_create_time = $this->filter_create_time($today_time);
        $result_type_ids=array("1","100","300");
        
        $lastid = 0;
        $limit = 30;
        $product_stock_obj = app::get('material')->model('basic_material_stock');
        $product_obj = app::get('material')->model('basic_material');
        $reportProductNsaleObj = app::get('ome')->model('analysis_productnsale');
...
// 				$sql_filter[] = "type_id not in(".implode(",",$result_type_ids).")";
				...
				$sql_str = "select ".implode(",",$arr_fields_colums)." from ".kernel::database()->prefix."ome_iostock 
							where ".implode(" AND ",$sql_filter)." group by bn";
				...
				foreach ($final_result_rows as $var_final_result){
					...
					$insert_arr["material_name"] = $material_bn_for_name_barcode[$var_final_result['material_bn']]['material_name'];
	...
	// 				$insert_arr['nsale_days'] = floor(($today_time-intval($var_final_result["create_time"]))/86400);
					$old = $reportProductNsaleObj->db_dump(['material_bn'=>$insert_arr['material_bn']], 'nsale_id');
					if($old['nsale_id']) {
						$reportProductNsaleObj->update($insert_arr, ['nsale_id'=>$old['nsale_id']]);
					} else {
						$reportProductNsaleObj->insert($insert_arr);
					}
				}
        	}
    ...
    private function filter_create_time($today_time){
    	...
    	if(!$omeanalysts_app->getConf("report_product_nsale_days")){
    		$omeanalysts_app->setConf("report_product_nsale_days", 90);//default 90 days
    	}
